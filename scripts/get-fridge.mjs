async function getFridge() {
  const url = `https://commons.wikimedia.org/w/api.php?action=query&format=json&list=search&srsearch=refrigerator+door+kitchen&srnamespace=6&srlimit=8`;
  const res = await fetch(url, { headers: { 'User-Agent': 'ACRepairWeb/1.0' } });
  const data = await res.json();
  const results = data.query ? data.query.search : [];
  console.log(`Results for refrigerator:`);
